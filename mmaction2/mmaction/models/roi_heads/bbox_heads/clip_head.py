...
class CLIPHeadAVA(nn.Module):
    ...
    def __init__(
        ...
        super(CLIPHeadAVA, self).__init__()
        assert temporal_pool_type in ['max', 'avg']
        assert spatial_pool_type in ['max', 'avg']
        clip_model, _ = clip.load('/opt/data/private/Chaotic_World/clip/ViT-B-16.pt', device='cpu', jit=False)
        clip_model.eval()
        for param in clip_model.parameters():
            param.requires_grad = False
        label_map, _ = read_labelmap(open('/opt/data/private/Chaotic_World/AR_ava_format/list_action.pbtxt'))
        ...
        self.video_text_head  = TextLogit(in_channels, text_dim, text_dim)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.view(x.size(0), -1)
        logits = self.video_text_head(x, text_feats)
