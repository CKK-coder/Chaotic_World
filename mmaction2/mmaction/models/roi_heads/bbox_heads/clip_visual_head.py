...
import clip
from mmaction.models.recognizers.coop import PromptLearner, TextEncoder
...
class TextLogit(nn.Module):
    def __init__(self, video_dim, text_dim, embed_dim):
        super().__init__()
        self.query_fc = nn.Sequential(
            nn.Linear(video_dim, embed_dim), nn.ReLU(inplace=True),
            nn.Linear(embed_dim, embed_dim))
        self.sentence_fc = nn.Sequential(
            nn.Linear(text_dim, embed_dim), nn.ReLU(inplace=True),
            nn.Linear(embed_dim, embed_dim))
        # self.sentence_fc = nn.Sequential(
    ...
    def get_sentence_query_logits(self, query_cls_feat, sentece_cls_feat, query_mask=None, sentence_mask=None):
        ...
        # sentece_cls_feat = self.sentence_fc(sentece_cls_feat) + sentece_cls_feat  # TODO
        query_cls_feat = query_cls_feat / (query_cls_feat.norm(dim=-1, keepdim=True)+1e-6)
        sentece_cls_feat = sentece_cls_feat / (sentece_cls_feat.norm(dim=-1, keepdim=True)+1e-6)
        logit = query_cls_feat @ sentece_cls_feat.transpose(0, 1)
        return logit
...
class CrossAttention(nn.Module):
    def __init__(self, depth, dim, heads):
        ...
        for _ in range(depth):
            self.crossattention.append(nn.MultiheadAttention(dim, heads))
    def forward(self, q, k ,v):
        for layer in self.crossattention:
            q,_ = layer(q,k,v)
        return q
        
class CLIPVHeadAVA(nn.Module):
    ...
    def __init__(
            ...
            mlp_head: bool = False,
            coop_n_ctx: int = 4,
            coop_ctx_init: str = 'The human is performing',
            coop_csc: bool = False,
            extend: bool = True,
            clip_model = '/opt/data/private/Chaotic_World/clip/ViT-B-16.pt',
            label = '/opt/data/private/Chaotic_World/AR_ava_format/list_action.pbtxt',
            extention = '/opt/data/private/Chaotic_World/AR_ava_format/label_extend.json'
            ) -> None:
        super(CLIPVHeadAVA, self).__init__()
        assert temporal_pool_type in ['max', 'avg']
        assert spatial_pool_type in ['max', 'avg']
        clip_model, pre = clip.load(clip_model, device='cpu', jit=False)
        clip_model.eval()
        for param in clip_model.parameters():
            param.requires_grad = False
        label_map, _ = read_labelmap(open(label))
        ...
        if not extend:
            ...
            self.prompt_learner = PromptLearner(coop_n_ctx, self.coop_ctx_init, classnames, clip_model, coop_csc=coop_csc)
        else:
            self.coop_ctx_init = ""
            with open(extention, 'r') as f:
                extend_text = json.load(f) 
            for cls in classnames:
                extend_pro.extend(extend_text[cls])
            self.prompt_learner = PromptLearner(coop_n_ctx, self.coop_ctx_init, extend_pro, clip_model, coop_csc=coop_csc)
        ...
        self.video_text_head  = TextLogit(in_channels*2, text_dim, text_dim)

        self.decoder1 = CrossAttention(3, in_channels, 8)
        self.decoder2 = CrossAttention(3, in_channels, 8)

        self.encoder = nn.Linear(512, 768)
        self.ffn = nn.Sequential(
            nn.Linear(768*2, 768), nn.ReLU(inplace=True),
            nn.Linear(768, 768))
    ...
    def forward(self, x: Tensor, key_img: Tensor, rois: Tensor, global_feats: Tensor) -> Tensor:
        ...
        global_feats = global_feats.squeeze(2)
        b,c,h,w = global_feats.size()
        global_feats = global_feats.view(h*w, b, c)
        roi_idx = rois[:,0]
        _, roi_nums = torch.unique(roi_idx, return_counts=True)
        clip_visuals = self.image_encoder(self.preprocess(key_img))
        clip_visuals = self.encoder(clip_visuals)
        out = []
        for batch in range(b):
            roi_num = roi_nums[batch]
            roi_feat = x[:roi_num,:]
            roi_feat = roi_feat.view(roi_feat.size(0), -1).unsqueeze(1)
            global_feat = global_feats[:,batch,:].unsqueeze(1)
            clip_visual = clip_visuals[batch,:].unsqueeze(0).unsqueeze(0)
            x1 = self.decoder1(roi_feat, global_feat, global_feat)
            x2 = self.decoder2(roi_feat, clip_visual, clip_visual)
            x_final = torch.cat([x1,x2],dim=-1)
            out.append(x_final)
        out = torch.cat(out, dim=0).squeeze(1)
        out = self.ffn(out)
        # q = torch.repeat_interleave(q,roi_nums,dim=0)
        if not self.dropout_before_pool and self.dropout_ratio > 0:
            x = self.dropout(x)
        prompts = self.prompt_learner() 
        tokenized_prompts = self.tokenized_prompts  
        text_feats = self.text_encoder(prompts, tokenized_prompts)  # [50*3, 512]
        x = x.view(x.size(0), -1)
        new_x = torch.cat([x,out],dim=1)
        logits = self.video_text_head(new_x, text_feats)
        logits = logits.view(logits.size(0),self.num_classes-1,-1)
        logits = logits.max(dim=-1).values
        logits = 10*logits
        cls_score = self.fc_cls(x)
        logits = torch.cat([logits, cls_score[:, 0:1]], dim=1)
        return (cls_score+logits)/2
