...
class AVADataset(BaseActionDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        # ckk start
        fin_fnums = list_from_file(self.frame_file)
        video_nums = dict()
        for line in fin_fnums:
            line_split = line.strip().split(',')
            video_nums[line_split[0]] = int(line_split[1])
        # end
        for line in fin:
            line_split = line.strip().split(',')
            ...
            img_key = f'{video_id},{timestamp:06d}'
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        data_info['filename_tmpl'] = data_info['video_id']+self.filename_tmpl
...
class AVAKineticsDataset(BaseActionDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        fin = list_from_file(self.ann_file)
        # ckk start
        fin_fnums = list_from_file('/opt/data/private/Chaotic_World/AR_ava_format/frame_num.csv')
        video_nums = dict()
        for line in fin_fnums:
            line_split = line.strip().split(',')
            video_nums[line_split[0]] = int(line_split[1])
        ...
        for line in fin:
            ...
            # end
            shot_info = (1, (end - start) * self._FPS)
            video_info = dict(
                video_id=video_id,
                timestamp=timestamp,
                entity_box=entity_box,
